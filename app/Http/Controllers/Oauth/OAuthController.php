...
class OAuthController extends Controller
{
    ...
    public function token(Request $request)
    {
        ...
        if ($authCode->cliente_id) {
            ...
            // Obtener roles del usuario directamente (Spatie)
            $rolesUsuario = $usuario->getRoleNames(); // Collection

            if ($rolesUsuario->isNotEmpty()) {
                $rol = $rolesUsuario->join(', ');
            } elseif ($usuario->empleado) {
                // fallback a empleado, si es que usas Spatie en empleado
                if (method_exists($usuario->empleado, 'getRoleNames')) {
                    $rolesEmpleado = $usuario->empleado->getRoleNames();
                    if ($rolesEmpleado->isNotEmpty()) {
                        $rol = $rolesEmpleado->join(', ');
                    }
